...
class Scraper {
    ...
    static void parseZastavky() {
        ...
        for (Element linka : l) {
            ...
            if (cisloLinky != null && !cisloLinky.text().equals("44")) {
                ...
                // System.out.println("cislo l:" + cisloLinky.text());
    ...
    static CasSpoja scrapeCas(String url, String vychodzia, String konecna, String smer,
                              int hodina, int minuta) {
        //System.out.println("---------------------------------------------------------");
        // System.out.println("h/m" + hodina + "/" + minuta);
        ...
        e4 = d.selectFirst(s);
        Element casy = d.selectFirst("td.cp_odchody_tabulka:has(td[class*='najblizsi'])");
        // System.out.println("----------------------------");
        //System.out.println(casy);
        //System.out.println("----------------------------");

        CasSpoja temp = new CasSpoja();
        // System.out.println(temp.hodina);
        if (casy != null) {
            //System.out.println("casVSpoji: " + e4.selectFirst("td").text());
            for (int i = hodina; i < 23; i++) {


                if (temp.hodina != -1) {
                    break;
                }
                Element hodinaRow = casy.selectFirst("tr.cp_odchody:has(td.cp_hodina:contains(" +
                        i + "))");
                //System.out.println("td.cp_hodina:contains(" + hodina + ")");
                if (hodinaRow != null) {
                    // System.out.println("got here: 1");
                    Elements minuty = hodinaRow.select("td:gt(0)");
                    //System.out.println("minuty: " + minuty);
                    if (minuty != null) {
                        //System.out.println("got here: 2");
                        for (Element min : minuty) {
                            if (min.text().length() > 1) {
                                // System.out.println("m" +min.text().substring(0,2));
                                if (Integer.parseInt(min.text().substring(0, 2)) >= minuta) {
                                    // System.out.println("got here: 3");
                                    temp.hodina = i;
                                    temp.minuta = Integer.parseInt(min.text().substring(0, 2));
                                    if (e4 != null) {
                                        temp.casVSpoji = Integer.parseInt(e4.selectFirst("td")
                                                .text());
                                    } else {
                                        temp.hodina = -1;
                                    }

                                    //System.out.println("Cvsp: " +  e4.selectFirst("td").text());
                                    // System.out.println("Time success: h = " + temp.hodina + "m = " +
                                    // temp.minuta);
                                    break;
                                }
                            }
        ...
        if (temp.minuta == -1 && temp.hodina == -1) {
            //System.out.println("Scaper method exit, temp-null");
            temp = null;
        }
